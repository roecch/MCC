...
def __setNullTables__(cur):
    ...
    for player in all_players:
        ...
        if (player,) not in playerExists:
            createQuery = ['CREATE TABLE IF NOT EXISTS ', player, '(MCCNUM VARCHAR(20), COLOR VARCHAR(20), ACERACE_TIME TIME, ACERACE_PLACE INT, BB_KILLS '
                                                                  'INT, BB_WINS INT, BINGO VARCHAR(20), BM VARCHAR(20), GR VARCHAR(20), HITW TIME, '
                                                                  'PT_HUNTER VARCHAR (20), PT_RUNNER VARCHAR(20), PW_FIRST VARCHAR(20), PW_COURSE '
                                                                  'VARCHAR(20), SB_SURVIVAL TIME, SB_KILLS INT, SOT_SOLO VARCHAR(20), SKYBLOCKE VARCHAR('
                                                                  '20), SG_SURVIVAL INT, SG_KILLS INT, TGTTOS_1 INT, TGTTOS_2 INT, TGTTOS_3 INT, TGTTOS_4 '
                                                                  'INT, TGTTOS_5 INT, TGTTOS_6 INT);']
            query = "".join(createQuery)
            cur.execute(query)

            for num in range(20):
                setAllNull = ['INSERT INTO ', player,
                              ' VALUES (', str(num + 1), ', null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, '
