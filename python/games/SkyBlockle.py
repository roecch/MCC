...
class SkyBlockle(SurvivalInterface, KillsInterface):
    def __init__(self, cur):
        self.cur = cur
        self.survPts = {"1-8" + cur_latest_event: (10, 70)}
        self.killPts = {"1-8" + cur_latest_event: 150}
    ...
    def perSurvivalAndFinal(self, game, survPt, finalPt, mcc_num, team) -> int:
        ...
        for rnd in game:
            ...
            place = int(rnd)
            total += place * survPt
            query = "SELECT MAX(TEAM) FROM MCCDATA WHERE SKYB_KILLS IS NOT NULL AND MCCNUM = " + mcc_num + ";"
    ...
    def calc(self, cur, player: str, scoring_type: str, extra_query: str) -> int:
        query = "SELECT MCCNUM, TEAM, SKYB_KILLS, SKYB_SURVIVAL " \
                "FROM MCCDATA WHERE SKYB_KILLS IS NOT NULL AND PLAYER = player" + extra_query
